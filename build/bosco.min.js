...
_this.elapsedTime+=delta;if(_this.elapsedTime>1){bosco.fps=_this.totalFrames;_this.totalFrames=0;_this.elapsedTime=0}var controllers=_this.controllers;for(var i=0,l=controllers.length;i<l;i++)controllers[i].update(delta);_this.renderer.render(_this.stage);Input.update();if(window["TWEEN"])TWEEN.update();requestAnimationFrame(_this.update);if(stats)stats.end()};this.resize=function(){var ratio=Math.min(window.innerWidth/_this.config.width,window.innerHeight/_this.config.height);_this.config.scale=
ratio;_this.stage.scale.x=_this.stage.scale.y=ratio;_this.renderer.resize(Math.ceil(_this.config.width*ratio),Math.ceil(_this.config.height*ratio))};_game=this;this.config=bosco.config=config;this.resources=bosco.resources=resources;this.previousTime=0;this.controllers=[];var renderer=this.renderer=PIXI.autoDetectRenderer(config.width,config.height,config.options);renderer.view.style.position="absolute";renderer.view.style.top="0px";renderer.view.style.left="0px";var stage=this.stage=new Container;
bosco.viewContainer=this.sprites=new Container;bosco.foreContainer=this.foreground=new Container;this.resize();document.body.appendChild(renderer.view);if(config.stats){var stats=this.stats=new Stats;stats.setMode(0);stats.domElement.style.position="absolute";stats.domElement.style.left="0px";stats.domElement.style.top="0px";document.body.appendChild(stats.domElement)}window.addEventListener("resize",this.resize,true);window.onorientationchange=this.resize;stage.addChild(this.sprites);stage.addChild(this.foreground);
if(config.ezgui&&config.theme)EZGUI.Theme.load(["assets/"+config.theme+"-theme/"+config.theme+"-theme.json"],function(){bosco.controller("main");requestAnimationFrame(_this.update)});else{bosco.controller("main");requestAnimationFrame(this.update)}}return Game}();bosco.Game=Game})(bosco||(bosco={}));
