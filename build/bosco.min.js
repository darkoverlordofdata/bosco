...
event.targetTouches[0]:event;if(_this.isFullScreen===undefined)_this.checkFullScreen();_this.mouseDown=true;_this.mouseButtonDown=true;_this.mousePosition.x=parseInt(event.clientX);_this.mousePosition.y=parseInt(event.clientY);return true};this.onTouchMove=function(event){event=event.targetTouches?event.targetTouches[0]:event;console.log("move",event.clientX,event.clientY);_this.mousePosition.x=parseInt(event.clientX);_this.mousePosition.y=parseInt(event.clientY)};this.onTouchEnd=function(event){_this.mouseDown=
false;_this.mouseButtonDown=false};document.addEventListener("touchstart",this.onTouchStart,true);document.addEventListener("touchmove",this.onTouchMove,true);document.addEventListener("touchend",this.onTouchEnd,true);document.addEventListener("mousedown",this.onTouchStart,true);document.addEventListener("mousemove",this.onTouchMove,true);document.addEventListener("mouseup",this.onTouchEnd,true);window.addEventListener("keydown",this.onKeyDown,true);window.addEventListener("keyup",this.onKeyUp,true)}
Object.defineProperty(Input,"mousePosition",{get:function(){return Input._input.mousePosition},enumerable:true,configurable:true});Input.getKeyDown=function(k){return Input._input.isDown(k.charCodeAt(0))};Input.getKeyUp=function(k){return Input._input.isUp(k.charCodeAt(0))};Input.getMouseButtonUp=function(m){return!Input._input.mouseButtonDown};Input.getMouseButton=function(m){return Input._input.mouseDown};Input.getMouseButtonDown=function(m){return Input._input.mouseButtonDown};Input.update=function(){Input._input.mouseDown=
