...
(function(bosco){var Sprite=PIXI.Sprite;var Texture=PIXI.Texture;var Container=PIXI.Container;var Input=bosco.utils.Input;(function(ScaleType){ScaleType[ScaleType["FILL"]=0]="FILL";ScaleType[ScaleType["FIXED"]=1]="FIXED"})(bosco.ScaleType||(bosco.ScaleType={}));var ScaleType=bosco.ScaleType;bosco.fps=0;var _prefabs={};function start(config){if(bosco.Properties&&config.properties)bosco.Properties.init(config.namespace,config.properties);for(var asset in config.assets)PIXI.loader.add(asset,config.assets[asset]);
PIXI.loader.load(function(loader,resources){new Game(config,resources)})}bosco.start=start;var DummyStats=function(){function DummyStats(){}DummyStats.prototype.begin=function(){};DummyStats.prototype.end=function(){};return DummyStats}();function buildComposite(config,level){if(level===void 0)level=0;if(Array.isArray(config)){var container=new Sprite;for(var i=0,l=config.length;i<l;i++)container.addChild(buildComposite(config[i],level+1));return container}else{var sprite=new Sprite(Texture.fromFrame(config.path));
for(var k in config)switch(k){case "anchor":sprite.anchor.set(config.anchor.x,config.anchor.y);break;case "scale":if(level>0)sprite.scale.set(config.scale.x,config.scale.y);break;case "position":if(level>0)sprite.position.set(config.position.x,config.position.y);break;case "rotation":if(level>0)sprite.rotation=config.rotation.z;break;case "tint":sprite.tint=config.tint;break}return sprite}}function prefab(name,parent){if(parent===void 0)parent=viewContainer;var sprite=new Sprite(_prefabs[name]);var config=
bosco.config.resources[name];for(var k in config)switch(k){case "anchor":sprite.anchor.set(config.anchor.x,config.anchor.y);break;case "scale":sprite.scale.set(config.scale.x,config.scale.y);break;case "position":sprite.position.set(config.position.x,config.position.y);break;case "rotation":sprite.rotation=config.rotation.z;break}if(parent)parent.addChild(sprite);return sprite}bosco.prefab=prefab;var Game=function(){function Game(config,resources){var _this=this;this.totalFrames=0;this.elapsedTime=
0;this.update=function(time){var stats=_this.stats;if(stats)stats.begin();var temp=_this.previousTime||time;_this.previousTime=time;var delta=bosco.delta=(time-temp)*.001;_this.totalFrames++;_this.elapsedTime+=delta;if(_this.elapsedTime>1){bosco.fps=_this.totalFrames;_this.totalFrames=0;_this.elapsedTime=0}var controllers=_this.controllers;for(var i=0,l=controllers.length;i<l;i++)controllers[i].update(delta);_this.renderer.render(_this.stage);Input.update();TWEEN.update();requestAnimationFrame(_this.update);
if(stats)stats.end()};this.resize=function(){var ratio=Math.min(window.innerWidth/_this.config.width,window.innerHeight/_this.config.height);_this.config.scale=ratio;_this.stage.scale.x=_this.stage.scale.y=ratio;_this.renderer.resize(Math.ceil(_this.config.width*ratio),Math.ceil(_this.config.height*ratio))};this.config=bosco.config=config;this.resources=resources;this.previousTime=0;var controllers=this.controllers=[];var renderer=this.renderer=PIXI.autoDetectRenderer(config.width,config.height,config.options);
for(var name in config.resources){var s=buildComposite(config.resources[name]);_prefabs[name]=s.generateTexture(renderer)}renderer.view.style.position="absolute";renderer.view.style.top="0px";renderer.view.style.left="0px";var stage=this.stage=new Container;viewContainer=this.sprites=new Container;foreContainer=this.fore=new Container;this.resize();document.body.appendChild(renderer.view);if(config.stats){var stats=this.stats=new Stats;stats.setMode(0);stats.domElement.style.position="absolute";stats.domElement.style.left=
"0px";stats.domElement.style.top="0px";document.body.appendChild(stats.domElement)}window.addEventListener("resize",this.resize,true);window.onorientationchange=this.resize;stage.addChild(this.sprites);stage.addChild(this.fore);for(var _i=0,_a=config.controllers;_i<_a.length;_i++){var className=_a[_i];var Class=window[config.namespace][className];controllers.push(new Class)}for(var _b=0;_b<controllers.length;_b++){var controller=controllers[_b];controller.start()}requestAnimationFrame(this.update)}
