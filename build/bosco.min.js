...
event.targetTouches[0]:event;if(bosco.isMobile()||bosco.config.fullScreen)try{if(document.documentElement["requestFullscreen"])document.documentElement["requestFullscreen"]();else if(document.documentElement["mozRequestFullScreen"])document.documentElement["mozRequestFullScreen"]();else if(document.documentElement["webkitRequestFullscreen"])document.documentElement["webkitRequestFullscreen"]();else if(document.documentElement["msRequestFullscreen"])document.documentElement["msRequestFullscreen"]()}catch(e){}_this.mouseDown=
true;_this.mouseButtonDown=true;_this.mousePosition.x=parseInt(event.clientX);_this.mousePosition.y=parseInt(event.clientY);return true};this.onTouchMove=function(event){event=event.targetTouches?event.targetTouches[0]:event;_this.mousePosition.x=parseInt(event.clientX);_this.mousePosition.y=parseInt(event.clientY)};this.onTouchEnd=function(event){_this.mouseDown=false;_this.mouseButtonDown=false};document.addEventListener("touchstart",this.onTouchStart,true);document.addEventListener("touchmove",
this.onTouchMove,true);document.addEventListener("touchend",this.onTouchEnd,true);document.addEventListener("mousedown",this.onTouchStart,true);document.addEventListener("mousemove",this.onTouchMove,true);document.addEventListener("mouseup",this.onTouchEnd,true);window.addEventListener("keydown",this.onKeyDown,true);window.addEventListener("keyup",this.onKeyUp,true)}Object.defineProperty(Input,"mousePosition",{get:function(){return Input._input.mousePosition},enumerable:true,configurable:true});Input.getKeyDown=
function(k){return Input._input.isDown(k.charCodeAt(0))};Input.getKeyUp=function(k){return Input._input.isUp(k.charCodeAt(0))};Input.getMouseButtonUp=function(m){return!Input._input.mouseButtonDown};Input.getMouseButton=function(m){return Input._input.mouseDown};Input.getMouseButtonDown=function(m){return Input._input.mouseButtonDown};Input.update=function(){Input._input.mouseDown=false;Input._input.states={}};Input._input=new Input;return Input}();utils.Input=Input})(utils=bosco.utils||(bosco.utils=
{}))})(bosco||(bosco={}));var bosco;
...
if(config.stats){var stats=this.stats=new Stats;stats.setMode(0);stats.domElement.style.position="absolute";stats.domElement.style.left="0px";stats.domElement.style.top="0px";document.body.appendChild(stats.domElement);this.update=function(time){stats.begin();temp=previousTime||time;previousTime=time;var delta=bosco.delta=(time-temp)*.001;for(var i=0,l=controllers.length;i<l;i++)controllers[i].update(delta);renderer.render(stage);stats.end();requestAnimationFrame(_this.update);Input.update();TWEEN.update()}}else this.update=
function(time){temp=previousTime||time;previousTime=time;var delta=bosco.delta=(time-temp)*.001;for(var i=0,l=controllers.length;i<l;i++)controllers[i].update(delta);renderer.render(stage);requestAnimationFrame(_this.update);Input.update();TWEEN.update()};window.addEventListener("resize",this.resize,true);window.onorientationchange=this.resize;stage.addChild(this.sprites);stage.addChild(this.fore);for(var _i=0,_a=config.controllers;_i<_a.length;_i++){var className=_a[_i];var Class=window[config.namespace][className];
