...
(function(bosco){var Sprite=PIXI.Sprite;var Texture=PIXI.Texture;function isMobile(){return/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}bosco.isMobile=isMobile;function prefab(name,bake){if(bake===void 0)bake=true;var s=_prefab(name);s.cacheAsBitmap=bake;return s}bosco.prefab=prefab;function _prefab(name){var config=bosco.config.resources[name];if(Array.isArray(config)){var container=new Sprite;for(var i=0,l=config.length;i<l;i++)container.addChild(prefab(config[i]));
return container}else{var sprite=new Sprite(Texture.fromFrame(config.path));for(var k in config)switch(k){case "anchor":sprite.anchor.set(config.anchor.x,config.anchor.y);break;case "scale":sprite.scale.set(config.scale.x,config.scale.y);break;case "position":sprite.position.set(config.position.x,config.position.y);break;case "rotation":sprite.rotation=config.rotation.z;break;case "tint":sprite.tint=config.tint;break}return sprite}}})(bosco||(bosco={}));var bosco;
