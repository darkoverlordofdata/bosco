...
(function(bosco){var Sprite=PIXI.Sprite;var Texture=PIXI.Texture;var Input=bosco.utils.Input;var Container=PIXI.Container;var Properties=bosco.Properties;bosco.fps=0;var _game;function controller(name){var args=[];for(var _i=1;_i<arguments.length;_i++)args[_i-1]=arguments[_i];if(!bosco.config.controllers[name])return;for(var _a=0,_b=_game.controllers;_a<_b.length;_a++){var controller=_b[_a];controller.stop()}_game.controllers=[];var root=bosco.config.controllers[name];root=Array.isArray(root)?root:
[root];for(var _c=0;_c<root.length;_c++){var className=root[_c];var Class=window[bosco.config.namespace][className];_game.controllers.push(new Class)}for(var _d=0,_e=_game.controllers;_d<_e.length;_d++){var controller=_e[_d];controller.start.apply(controller,args)}}bosco.controller=controller;function start(config){if(config.properties)Properties.init(config.namespace,config.properties);for(var asset in config.assets)PIXI.loader.add(asset,config.assets[asset]);PIXI.loader.load(function(loader,resources){return new Game(config,
resources)})}bosco.start=start;function prefab(name,parent){if(parent===void 0)parent=bosco.viewContainer;var s=_prefab(bosco.config.resources[name]);if(parent)parent.addChild(s);return s}bosco.prefab=prefab;function _prefab(config){if(Array.isArray(config)){var container=new Sprite;for(var i=0,l=config.length;i<l;i++)container.addChild(_prefab(config[i]));return container}else{var sprite=new Sprite(Texture.fromFrame(config.path));for(var k in config)switch(k){case "anchor":sprite.anchor.set(config.anchor.x,
config.anchor.y);break;case "scale":sprite.scale.set(config.scale.x,config.scale.y);break;case "position":sprite.position.set(config.position.x,config.position.y);break;case "rotation":sprite.rotation=config.rotation;break;case "tint":sprite.tint=config.tint;break}return sprite}}var Game=function(){function Game(config,resources){var _this=this;this.totalFrames=0;this.elapsedTime=0;this.tween=false;this.input=false;this.update=function(time){var stats=_this.stats;if(stats)stats.begin();var temp=_this.previousTime||
time;_this.previousTime=time;var delta=bosco.delta=(time-temp)*.001;_this.totalFrames++;_this.elapsedTime+=delta;if(_this.elapsedTime>1){bosco.fps=_this.totalFrames;_this.totalFrames=0;_this.elapsedTime=0}var controllers=_this.controllers;for(var i=0,l=controllers.length;i<l;i++)controllers[i].update(delta);_this.renderer.render(_this.stage);if(_this.input)Input.update();if(_this.tween)TWEEN.update();requestAnimationFrame(_this.update);if(stats)stats.end()};this.resize=function(){var ratio=Math.min(window.innerWidth/
_this.config.width,window.innerHeight/_this.config.height);_this.config.scale=ratio;_this.stage.scale.x=_this.stage.scale.y=ratio;_this.renderer.resize(Math.ceil(_this.config.width*ratio),Math.ceil(_this.config.height*ratio))};_game=this;this.config=bosco.config=config;this.resources=bosco.resources=resources;this.tween=config.tween;this.input=config.input;this.previousTime=0;this.controllers=[];var renderer=this.renderer=PIXI.autoDetectRenderer(config.width,config.height,config.options);renderer.view.style.position=
"absolute";renderer.view.style.top="0px";renderer.view.style.left="0px";var stage=this.stage=new Container;bosco.viewContainer=this.sprites=new Container;bosco.foreContainer=this.foreground=new Container;this.resize();document.body.appendChild(renderer.view);if(config.stats){var stats=this.stats=new Stats;stats.setMode(0);stats.domElement.style.position="absolute";stats.domElement.style.left="0px";stats.domElement.style.top="0px";document.body.appendChild(stats.domElement)}window.addEventListener("resize",
this.resize,true);window.onorientationchange=this.resize;stage.addChild(this.sprites);stage.addChild(this.foreground);if(config.ezgui&&config.theme)EZGUI.Theme.load(["assets/"+config.theme+"-theme/"+config.theme+"-theme.json"],function(){bosco.controller("main");requestAnimationFrame(_this.update)});else{bosco.controller("main");requestAnimationFrame(this.update)}}return Game}();bosco.Game=Game})(bosco||(bosco={}));
