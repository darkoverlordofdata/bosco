...
bosco.foreContainer=this.foreground=new Container;this.resize();document.body.appendChild(renderer.view);if(config.stats){var stats=this.stats=new Stats;stats.setMode(0);stats.domElement.style.position="absolute";stats.domElement.style.left="0px";stats.domElement.style.top="0px";document.body.appendChild(stats.domElement)}window.addEventListener("resize",this.resize,true);window.onorientationchange=this.resize;stage.addChild(this.sprites);stage.addChild(this.foreground);if(config.ezgui&&config.theme)EZGUI.Theme.load(["assets/"+
config.theme+"-theme/"+config.theme+"-theme.json"],function(){bosco.controller("main");requestAnimationFrame(_this.update)});else{bosco.controller("main");requestAnimationFrame(this.update)}}return Game}();bosco.Game=Game})(bosco||(bosco={}));
